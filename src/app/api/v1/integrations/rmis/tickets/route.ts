...
export async function POST(request: Request) {
  ...
  if (!project) {
    return response(
      { error: { code: "PROJECT_UNAVAILABLE", message: "The RMIS project is missing or inactive." } },
      503,
    );
  }
  ...
  const [requester] = await db
    ...
    .where(
      and(
        eq(users.email, parsed.data.requesterEmail.toLowerCase()),
  ...
  if (!requester) {
    return response(
      { error: { code: "REQUESTER_FORBIDDEN", message: "The requester cannot create RMIS tickets." } },
      403,
    );
  }
  ...
  if (requester.role !== "admin") {
    const [membership] = await db
      ...
      .from(userProjects)
      .where(and(eq(userProjects.userId, requester.id), eq(userProjects.projectId, project.id)))
      .limit(1);
  ...
  try {
    ...
    return response(
      {
        data: {
          ticket: {
            ...
            project: { id: project.id, code: project.name, title: project.title },
    ...
    console.error("RMIS integration ticket creation failed.", error);
    return response(
